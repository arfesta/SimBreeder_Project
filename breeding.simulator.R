...
create.map <- function(num.markers, totalQTL,  numSNPQTL, map.dist = "haldane",
  ...
  last.pos.chrs <-  sapply(1:12,function(x){round(map$pos[chromo.loci[x]-1],0)})
  Markers <- vector()
...
create.parents.phenos <- function(parents.TGV, h2, E.var = NULL, save = F,
                                  folder = NULL) {
...
####Create TGV2####
create.progeny.TGV <- function(proginfo, mapinfo=map, crossdesign=cross.design, A=Major.value,a=Minor.value, dom.coeff=Dominance.Coeff,
                               save=F, rep.num=NULL, prefix=NULL){
...
####Extract Selections####
extract.selections <- function(mapinfo=map, crossdesign=cross.design, past.tgv, past.phenos, parentinfo,
