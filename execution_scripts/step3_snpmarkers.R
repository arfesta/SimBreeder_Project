...
NumParents=64
prog.per.cross = 60
af.selection = "snp_markers" #ABLUP GBLUP_FI snpibd
...
for(pop.num in 3:10){
  ...
  the.name <- paste("SNPMARKERS.nodom.64.OP.pop_", pop.num, sep="")
  assign(x=the.name,value=newList)
  save(list=the.name,file=paste("/mnt/results/3k/64/snp_markers/",the.name,".RData",sep=""))
  print(pop.num)
}   
#    proc.time() - pt
#  },mc.cores=8)
