...
NumParents=64
prog.per.cross = 60
af.selection = "GBLUP_FI" #ABLUP GBLUP_FI snp_markers
...
#pop.num=1
    ...
    the.name <- paste("GBLUPFI.nodom.64.OP.pop_", pop.num, sep="")
    assign(x=the.name,value=newList)
    save(list=the.name,file=paste("/mnt/results/3k/64/snp_fi/",the.name,".RData",sep=""))
    print(pop.num)
}   
#    proc.time() - pt
#  },mc.cores=8)
