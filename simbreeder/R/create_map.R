...
#Create Map Function####
create_genetic_map <- function(num.chromos, map.length, num.markers, total.loci, total.QTL, num.SNPQTL, 
                               map.dist = "haldane", chromosome.size.range=.2,
                               signif.digits=2, het.markers=F, seed=F, save=F) {
  ...
  calc_rec_freq <- function(x,mapdist=map.dist,interval=intervals) {
    ...
    if(mapdist == "kosambi") { out <- ((exp(2 * loci.intervals) - exp(-2 * loci.intervals)) / (exp(2 * loci.intervals) + exp(-2 * loci.intervals))) / 2 }
    out
  }
  ...
  map <- data.frame(chr=unlist(list1),loci=locus.names, dist= as.numeric(unlist(intervals)),types= (rep("NA",total.loci)), MAF=rep("NA",total.loci), 
