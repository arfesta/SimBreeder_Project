library(SimBreeder)
setwd("~/Desktop/Simulator_draft/graham")
map <- create_genetic_map(num.chromos = 12,map.length = 1800,num.markers = 540,total.QTL = 2520,
                   num.SNPQTL = 1960,distribute.loci ="even",marker.distribution = "equally-spaced")
parents <- create_parents(map.info = map,num.parents = 50,max.delt.allele = 14,heterozygous.markers = F)

parent.tgv <- calc_parents_TGV(parents = parents,map.info = map,A = 1,a = -100,dom.coeff = 1)
parent.phenos <- sim_parents_phenos(parents.TGV = parent.tgv,h2 = .3)

first.gen.cross <- create_cross_design(parent.info = parents,mating.design = "cross.file.input",cross.file = "g.study.1stgen.txt",generation = 1)


first.prog <- make_crosses(parent.info = parents,map.info = map,cross.design = first.gen.cross,run.parallel = T,num.cores = 3)

first.prog.tgv <- calc_progeny_TGV(prog.info = first.prog,map.info = map,cross.design = first.gen.cross,A = 1,a = -100,dom.coeff = 1)
first.prog.phenos <- sim_progeny_phenos(progeny.TGV = first.prog.tgv,h2 = .3,cross.design = first.gen.cross)
...
prog.ex1 <- extract_selections(among.family.selection = "Phenotype",map.info = map,cross.design =  first.gen.cross,past.tgv = parent.tgv,past.phenos = parent.phenos,parent.info = parents,
                   progeny.info = first.prog,progeny.TGV = first.prog.tgv,progeny.phenos = first.prog.phenos,selection.strategy = "NULL",num.selections.within.family = 1, num.selections.among.family = 40,
                    )
selections <- sort(as.numeric(names(prog.ex1$selections)))
second.gen.cross <- read.table("g.2nd.gen.ped2.txt")
second.gen.cross <- cbind(selections[second.gen.cross[,1]],selections[second.gen.cross[,2]],second.gen.cross[,3])

#second.gen.cross <- read.table("g.2nd.gen.ped.txt")
...
second.prog <- make_crosses(parent.info = prog.ex1,map.info = map,cross.design = second.gen.cross,run.parallel = T,num.cores = 3)

second.prog.tgv <- calc_progeny_TGV(prog.info = second.prog,map.info = map,cross.design = second.gen.cross,A = 1,a = -100,dom.coeff = 1)
second.prog.phenos <- sim_progeny_phenos(progeny.TGV = second.prog.tgv,h2 = .3,cross.design = second.gen.cross)
{
