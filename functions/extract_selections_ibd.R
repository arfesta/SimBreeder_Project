...
extract_selections_ibd <- function(map.info, cross.design, past.tgv, past.phenos, parent.info, progeny.info, progeny.TGV, progeny.phenos,
                                   selection.strategy, among.family.selection, within.family.selection = NULL, useSNPs="NULL", useSNPfxn=0.25,
  ...
  {
    snp.marker.loci <- which(map.info$genetic.map$types =="m")
    #snp.marker.loci <- which(map.info$types == "m")
    snpqtl.loci <- map.info$QTLSNP.loci
    #snpqtl.loci <- which(map.info$types == "snpqtl")
    snpqtl.map <- map.info$genetic.map[snpqtl.loci,]
    ...
    if (generation == 1) {
      if (reduced) { 
        parent.markers <- parent.info$genos.3d[snp.marker.loci, (names(parent.info$mean.parent.phenos)), ]
      }  else {
  ...
  if (among.family.selection == "ABLUP") {
    ...
    the.data <- as(getAInv(ped),"CsparseMatrix")
  ...
  if (among.family.selection == "GBLUP_FI") {
    fi.marker.loci <- which(map.info$genetic.map$types == "fi")
    prog.markers <- progeny.info$genos.3d[fi.marker.loci, , ]
    ...
    the.data <- Rfast::spdinv(the.data)
    n.col <- ncol(the.data)
    h.2 <- var(prog.genetic.values)/var(prog.phenos)
    lambda <- (1 - h.2)/h.2
    I <- diag(n.col)
  ...
  if (among.family.selection == "snpibdfam") {
    ...
    # if(useSNPs=="subset") {
    #    SNP.subset.num <- floor(length(snp.marker.loci)*useSNPfxn)
    #    SNP.sample <- sample(snp.marker.loci,SNP.subset.num,replace=FALSE)
    #    snpmarker.map <- map.info$genetic.map[snp.marker.loci,]
    #    snpmarker.map$chr <- gsub("chr","",snpmarker.map$chr)
    # snpmarker.loci <- SNP.sample # Assign snpmarker.loci to just the subset of SNPQTL loci used for relationship matrix
    #} else {
    snpmarker.map <- map.info$genetic.map[snp.marker.loci,]
    snpmarker.map$chr <- gsub("chr","",snpmarker.map$chr)
    #  }
    ...
    X <- matrix(NA,nrow=0,ncol=length(snp.marker.loci))
    ...
    progeny.snpqtl.genos <- matrix(paste(t(progeny.info$genos.3d[snp.marker.loci,,1]),t(progeny.info$genos.3d[snp.marker.loci,,2]),sep=" "),
                                   nrow(prog.pedigree),length(snp.marker.loci))
    dimnames(progeny.snpqtl.genos) <- list(names(progeny.phenos$phenos),snpmarker.map$loci)
    ...
    if(generation==1){
      for(row in 1:nrow(uniq.fam.mtx)){
        ...
        par1.genos <- paste(t(parent.info$genos.3d[snp.marker.loci,uniq.fam.mtx[row,1],1]),t(parent.info$genos.3d[snp.marker.loci,uniq.fam.mtx[row,1],2]),sep=" ")
        par2.genos <- paste(t(parent.info$genos.3d[snp.marker.loci,uniq.fam.mtx[row,2],1]),t(parent.info$genos.3d[snp.marker.loci,uniq.fam.mtx[row,2],2]),sep=" ")
    ...
    write.table(current.ped,"./tmp.ped",row.names=FALSE,col.names=FALSE,quote=FALSE,sep=" ")
    plink.map <- data.frame(chr=snpmarker.map$chr,loci=snpmarker.map$loci,cMpos=snpmarker.map$pos,ntpos=(snpmarker.map$pos*1000000))
    write.table(plink.map,"./tmp.map",row.names=FALSE,col.names=FALSE,quote=FALSE,sep=" ")
    
    # Use system() to invoke plink v1.9 to convert text files to binary format
    system("plink1 --noweb --file  tmp --make-bed --out tmp",
           intern=FALSE)
    ...
    library(SNPRelate)
    snpgdsBED2GDS(bed.fn="./tmp.bed",bim.fn="./tmp.bim",
                  fam.fn="./tmp.fam", out.gdsfn="./tmp.gds")
    genofile <- snpgdsOpen("./tmp.gds")
    family.mtx <- matrix(unlist(strsplit((read.gdsn(index.gdsn(genofile,"sample.id"))),"-")),byrow=TRUE,ncol=2)
    #king.ibd <- snpgdsIBDKING(genofile,type="KING-robust",family.id=family.mtx[,1],num.thread = 10)
    king.ibd <- snpgdsIBDKING(genofile,type="KING-robust",num.thread = 10)
  ...
  if (among.family.selection == "snpibd") {
    ...
    #fam <- matrix(NA,nrow=0,ncol=6) 
    # Define empty matrix with 0 rows and enough cols of NA values for all snpqtl loci; cbind rows of space-separated allele calls to this matrix
    #X <- matrix(NA,nrow=0,ncol=length(snpmarker.loci))
    
    # Recover genotypes for just the SNPQTL loci for progeny from transposed progeny1$genos.3d array of all loci; 
    # paste the two alleles together with space. Loci now in columns of allele calls; samples in rows. Assign column
    # names of progeny IDs from progeny1.phenos$phenos object
    progeny.snpmarker.genos <- matrix(paste(t(progeny.info$genos.3d[snp.marker.loci,,1]),t(progeny.info$genos.3d[snp.marker.loci,,2]),sep=""),
                                      nrow(prog.pedigree),length(snp.marker.loci))
    
    progeny.snpmarker.genos[which(progeny.snpmarker.genos == "GG")] <- 0
    progeny.snpmarker.genos[which(progeny.snpmarker.genos == "gG")] <- 1
    progeny.snpmarker.genos[which(progeny.snpmarker.genos == "Gg")] <- 1
    progeny.snpmarker.genos[which(progeny.snpmarker.genos == "gg")] <- 2
    progeny.snpmarker.genos <- (apply(progeny.snpmarker.genos,1,as.numeric))
    
    dimnames(progeny.snpmarker.genos) <- list(snpmarker.map$loci,names(progeny.phenos$phenos))
    
    # load SNPRelate library, convert binary files to GDS format, then define GDS file as genofile and calculate KING-robust kinship 
    library(SNPRelate)
    snpgdsCreateGeno(gds.fn = "tmp2.gds",genmat = progeny.snpmarker.genos,snp.chromosome = snpmarker.map$chr,snp.id = snpmarker.map$loci)
    #snpgdsBED2GDS(bed.fn="./tmp_bin.bed",bim.fn="./tmp_bin.bim",
    #              fam.fn="./tmp_bin.fam", out.gdsfn="tmp2.gds")
    genofile <- snpgdsOpen("./tmp2.gds")
    #family.mtx <- matrix(unlist(strsplit((read.gdsn(index.gdsn(genofile,"sample.id"))),"-")),byrow=TRUE,ncol=2)
    king.ibd <- snpgdsIBS(genofile,num.thread = num.cores)
    king.mtx <- king.ibd$ibs
    dimnames(king.mtx) <- list(king.ibd$sample.id,king.ibd$sample.id)
    #king.ibd <- snpgdsGRM(genofile)
    snpgdsClose(genofile)
  ...
  if (among.family.selection == "snp_markers") {
    ...
    snpgdsCreateGeno(gds.fn = "tmp2.gds",genmat = progeny.snpmarker.genos,snp.chromosome = snpmarker.map$chr,snp.id = snpmarker.map$loci)
    genofile <- snpgdsOpen("./tmp2.gds")
    snp.ibs <- snpgdsIBS(genofile,num.thread = num.cores)
    snp.ibs.mtx <- snp.ibs$ibs
    dimnames(snp.ibs.mtx) <- list(snp.ibs$sample.id,snp.ibs$sample.id)
    snpgdsClose(genofile)
    
    l <- match(names(progeny.phenos$phenos), rownames(snp.ibs.mtx))
    rel.mat <- snp.ibs.mtx
    rm(snp.ibs.mtx,snp.ibs);gc(full=T)
    
    A <- as_tibble(as.matrix(getA(ped)))
    A = A[match(names(prog.phenos), colnames(A)), match(names(prog.phenos),  colnames(A))]
    the.data <- as.matrix(rel.mat)*.99 + as.matrix(A) * 0.01
    #rm(rel.mat,A); gc(full=T,reset = T,verbose = F)
    #the.datav2 <- apply(the.data,1,function(x){round(x,2)})
    #pt <- proc.time()
    #the.data4 <- Matrix::solve(the.data)
    #proc.time()-pt
    
    #pt <- proc.time()
    #the.data3 <- fastmatrix::lu(the.data)
    #the.data3 <- fastmatrix::lu2inv(the.data3)
    #proc.time()-pt
  ...
  }  
  {
    ...
    if(length(dim(new.parent.genos)) < 3){
      all.markers1 <- cbind(parent.info$genos.3d[snp.marker.loci,,1], new.parent.genos[snp.marker.loci, 1])
      colnames(all.markers1) <- c(colnames(parent.info$genos.3d[snp.marker.loci,,1]),select.ped.ids)
      all.markers2 <- cbind(parent.info$genos.3d[snp.marker.loci,,2], new.parent.genos[snp.marker.loci, 2])
      colnames(all.markers2) <- c(colnames(parent.info$genos.3d[snp.marker.loci,,2]),select.ped.ids)
