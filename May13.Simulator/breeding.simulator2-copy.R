...
extract.selections <- function(num.cores=1, mapinfo=map, crossdesign=cross.design, past.tgv, past.phenos, parentinfo,
  ...
  library(synbreed); library(MatrixModels); library(parallel); library(pedigreemm); library(pedigree)
  ...
  if(selection.strategy == "within.family"){
    if(within.family.selection == "GBLUP"){
      library(parallel)
      #First subset top families to select from
      top.fams <- top.280.families[1:numSelections.among.family]
      top.fams <- sort(top.fams)
      this.set <- vector()
      for(i in 1:length(top.fams)){ this.set <- c(this.set,seq((top.fams[i]*60)-59,top.fams[i]*60))}
      sub.p.phenos <- progeny.phenos[this.set]
      first <- 1
      last <- prog.percross
      prog.select.blups <- progeny.blups[this.set]
      prog.select <- vector()
      for(each in 1:numSelections.among.family) {
        prog.select <- c(prog.select,sort(prog.select.blups[first:last],decreasing=T)[1:30])
        first <- last+1
        last <- first+prog.percross-1}
      this.set <- match(names(prog.select), names(progeny.blups))
      sub.p.phenos <- progeny.phenos[this.set]
      s.this.set <- sort(as.numeric(names(sub.p.phenos)))
      s.match <-        match(s.this.set, as.numeric(names(sub.p.phenos)))
      sub.p.phenos <- sub.p.phenos[s.match]
      A <- as.matrix(getA(ped))
      l <- match(names(sub.p.phenos),rownames(A))
      A <- A[l,l]
      d.match <-  match(s.this.set, as.numeric(names(progeny.phenos)))
      
      m <- mapinfo$available.Markers
      if(gen==1){parentmarkers <- parentinfo$genos.3d[m,,]} else {parentmarkers <- parentinfo$all.markers}
      progmarkers <- progenyinfo$genos.3d[m,d.match,]
      allele1 <- cbind(parentmarkers[,,1],progmarkers[,,1])
      allele2 <- cbind(parentmarkers[,,2],progmarkers[,,2])
      #rm(progmarkers,parentmarkers)
      ...
      n.col <- ncol(allele1)
      g.rel.mat <- matrix(NA,nrow=n.col,ncol=n.col)
      yo <- mclapply(1:(n.col-1),FUN = function(x){
        com <- allele1[,x]
        com2 <- allele2[,x]
        ss <- x + 1
        b <- sapply(ss:ncol(allele1),FUN=function(x){
      ...
      g.rel.mat[lower.tri(g.rel.mat)] <- unlist(yo)
      rm(yo,allele1,allele2); gc()
      end <- as.numeric(dim(g.rel.mat)[1]-dim(progenyinfo$genos.3d[,d.match,])[2])
      G.Rel.Mat <- g.rel.mat[-c(1:end),-c(1:end)]
      # get the decomposition: G = UDU'
      G.Rel.Mat1 <- .97*G.Rel.Mat + .03*A
      rm(G.Rel.Mat,A)
      ...
      sol <- solve(rBind(cBind(n.col, t(rep(1,n.col))),
                         cBind(rep(1,n.col), (I + (lambda*out)))),
                   matrix(c(sum(sub.p.phenos),
                            c(as.vector(sub.p.phenos)))))
      sol <- sol[-1,1]
      names(sol) <- names(sub.p.phenos)
      gprogeny.blups <- sol
      ...
      for(family in 1:numSelections.among.family){
        num.offspring <- as.numeric(cross.design[family,3])/2
  ...
  Selection.phenos<- progeny.phenos[names(Selections)]
  new.pars.genval <- prog.genetic.values[names(Selections)]
  Selection.EBvs <- progeny.blups[names(Selections)]
