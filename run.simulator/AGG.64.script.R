...
#Load package####
  #library(Breeding.Simulator)
  pop.name <- paste("/media/titan/ARF/base.population/base.population.",pop.num,".RData",sep="")
  load(pop.name)
  OP.parent.phenos$pars <- OP.parent.phenos$pars[1:64]
  OP.parent.phenos$delt.alleles <- OP.parent.phenos$delt.alleles[1:64]
  OP.parent.phenos$genetic.values <- OP.parent.phenos$genetic.values[1:64]
  OP.parent.phenos$phenos <- OP.parent.phenos$phenos[1:64]
  OP.parent.phenos$genos.3d <- OP.parent.phenos$genos.3d[,1:64,]
  OP.parent.phenos$marker.matrix <- OP.parent.phenos$marker.matrix[1:64,]
...
#### Create 1st gen progeny#######
  ...
  cross.design <- create.cross.design(parentinfo = OP.parent.phenos,prog.percross = 60,gen = 1,use.op.par.phenos = T)
  progeny1 <- make.crosses(crossdesign = cross.design, parentinfo = parents, run.parallel = T,num.cores = 20)
  progeny1.TGV <- create.progeny.TGV(crossdesign = cross.design, proginfo = progeny1)
  set.seed(NULL); progeny1.phenos <- create.progeny.phenos(crossdesign = cross.design, progeny.TGV = progeny1.TGV, h2 = .3)
  progeny1.extractions <- extract.selections(crossdesign = cross.design, parentinfo = OP.parent.phenos, past.tgv = parents.TGV,past.phenos = parents.phenos, relmatrix = "markers",
                                             progenyinfo = progeny1,progenyTGV = progeny1.TGV,progenyphenos = progeny1.phenos,selection.strategy = "within.family",
                                             among.family.selection = "ABLUP", within.family.selection = "GBLUP", reduced = T,numSelections.among.family = 64)
...
####Generation 2#####
  ...
  cross.design <- create.cross.design(parentinfo = progeny1.extractions,prog.percross = 60,gen = 2)
  progeny2 <- make.crosses(crossdesign=cross.design,parentinfo = progeny1.extractions,run.parallel = T,num.cores = 20)
  progeny2.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny2)
  set.seed(NULL); progeny2.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny2.TGV, h2 = .3)
  progeny2.extractions <- extract.selections(crossdesign=cross.design, past.tgv = progeny1.TGV, relmatrix = "markers",
                                             past.phenos = progeny1.phenos, parentinfo = progeny1.extractions,progenyinfo = progeny2,
                                             progenyTGV = progeny2.TGV,progenyphenos = progeny2.phenos,selection.strategy = "within.family",
                                             among.family.selection = "ABLUP",within.family.selection = "GBLUP",numSelections.among.family = 64)
...
####Generations 3####
  ...
  cross.design <- create.cross.design(parentinfo = progeny2.extractions,prog.percross = 60,gen = 3)
  progeny3 <- make.crosses(crossdesign=cross.design,parentinfo = progeny2.extractions,run.parallel = T,num.cores = 20)
  progeny3.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny3)
  set.seed(NULL); progeny3.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny3.TGV,h2 = .3)
  progeny3.extractions <- extract.selections(crossdesign=cross.design, past.tgv = progeny2.TGV, relmatrix = "markers",
                                             past.phenos = progeny2.phenos, parentinfo = progeny2.extractions,progenyinfo = progeny3,
                                             progenyTGV = progeny3.TGV,progenyphenos = progeny3.phenos,selection.strategy = "within.family", 
                                             among.family.selection = "ABLUP",within.family.selection = "GBLUP",numSelections.among.family = 64)
...
####Generation 4####
  ...
  cross.design <- create.cross.design(parentinfo = progeny3.extractions,prog.percross = 60,gen = 4)
  progeny4 <- make.crosses(crossdesign=cross.design,parentinfo = progeny3.extractions,run.parallel = T,num.cores = 20)
  progeny4.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny4)
  set.seed(NULL); progeny4.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny4.TGV, h2 = .3)
  progeny4.extractions <- extract.selections(crossdesign=cross.design, relmatrix = "markers",
                                             ...
                                             selection.strategy = "within.family", 
                                             among.family.selection = "ABLUP",within.family.selection = "GBLUP",numSelections.among.family = 64)
...
####Generation 5####
  ...
  cross.design <- create.cross.design(parentinfo = progeny4.extractions,prog.percross = 60,gen = 5)
  progeny5 <- make.crosses(crossdesign=cross.design,parentinfo = progeny4.extractions,run.parallel = T,num.cores = 20)
  progeny5.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny5)
  set.seed(NULL); progeny5.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny5.TGV, h2 = .3)
  progeny5.extractions <- extract.selections(crossdesign=cross.design, relmatrix = "markers",
...
####Generation 6####
  ...
  cross.design <- create.cross.design(parentinfo = progeny5.extractions,prog.percross = 60,gen = 6)
  progeny6 <- make.crosses(crossdesign=cross.design,parentinfo = progeny5.extractions,run.parallel = T,num.cores = 20)
  progeny6.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny6)
  set.seed(NULL); progeny6.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny6.TGV, h2 = .3 )
  progeny6.extractions <- extract.selections(crossdesign=cross.design,relmatrix = "markers",
...
####Generation 7####
  ...
  cross.design <- create.cross.design(parentinfo = progeny6.extractions,prog.percross = 60,gen = 7)
  progeny7 <- make.crosses(crossdesign=cross.design,parentinfo = progeny6.extractions,run.parallel = T,num.cores = 20)
  progeny7.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny7)
  set.seed(NULL); progeny7.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny7.TGV, h2 = .3 )
  progeny7.extractions <- extract.selections(crossdesign=cross.design,relmatrix = "markers",
...
####Generation 8####
  ...
  cross.design <- create.cross.design(parentinfo = progeny7.extractions,prog.percross = 60,gen = 8)
  progeny8 <- make.crosses(crossdesign=cross.design,parentinfo = progeny7.extractions,run.parallel = T,num.cores = 20)
  progeny8.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny8)
  set.seed(NULL); progeny8.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny8.TGV, h2 = .3)
  progeny8.extractions <- extract.selections(crossdesign=cross.design,relmatrix = "markers",
...
####Generation 9####
  ...
  cross.design <- create.cross.design(parentinfo = progeny8.extractions,prog.percross = 60,gen = 9)
  progeny9 <- make.crosses(crossdesign=cross.design,parentinfo = progeny8.extractions,run.parallel = T,num.cores = 20)
  progeny9.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny9)
  set.seed(NULL); progeny9.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny9.TGV, h2 = .3)
  progeny9.extractions <- extract.selections(crossdesign=cross.design,relmatrix = "markers",
...
####Generation 10####
  ...
  cross.design <- create.cross.design(parentinfo = progeny9.extractions,prog.percross = 60,gen = 10)
  progeny10 <- make.crosses(crossdesign=cross.design,parentinfo = progeny9.extractions,run.parallel = T,num.cores = 20)
  progeny10.TGV <- create.progeny.TGV(crossdesign=cross.design,proginfo = progeny10)
  set.seed(NULL); progeny10.phenos <- create.progeny.phenos(crossdesign=cross.design,progeny.TGV = progeny10.TGV, h2 = .3 )
  progeny10.extractions <- extract.selections(crossdesign=cross.design, relmatrix = "markers",
...
#### List for output objects####
  ...
  save(list=the.name,file=paste("/media/titan/ARF/results/AGA.64/output/",the.name,".RDA",sep=""))
}
