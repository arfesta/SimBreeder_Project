...
# NOTE - this version of script saves selected progeny snpqtl marker genotypes for even-numbered generations to monitor homozygosity,
# in a list of matrices that is saved with the final dataset
## Run Values ####
NumParents=64
prog.per.cross = 60
af.selection = "ABLUP"
...
Dominance.Coeff=0
indiv.tree.h2 = .3
load("./25Oct_seeds.RData") 
# Uncomment line to copy seeds.vector to all_seeds - 25Oct_seeds is already called all_seeds
# all_seeds <- seed.vector

mclapply(1:30,function(pop.num){ # Change 118:300 to 1:30 
  ...
  save(newList,file=paste("/media/hd02/Rsimulations/SimBreeder_Project/results/64/ABLUP_NoClust/",the.name,".RData",sep="")) # Change path
  
  
  },mc.cores=60,mc.preschedule = F)
