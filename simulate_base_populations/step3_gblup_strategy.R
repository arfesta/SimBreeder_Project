...
for(pop.num in 2:10){
res <- mclapply(111:300,function(pop.num){
  this.gen <- all_seeds[[pop.num]]
  
    load.pop <- paste0("/mnt/data_v2/base_population_op_testing/base_population_op_test_",pop.num,".RData")
    load(load.pop)
  ...
  the.name <- paste("GBLUP.nodom.64.OP.pop_", pop.num, sep="")
  assign(x=the.name,value=newList)
  save(list=the.name,file=paste("./data_v2/results/64/GBLUP/",the.name,".RData",sep=""))
  
  
  },mc.cores=60,mc.preschedule = F)


  the.name <- paste("GBLUP.nodom.64.OP.pop", pop.num, sep="")
  assign(x=the.name,value=newList)
