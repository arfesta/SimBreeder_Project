# Run this on Titan ####
source('/media/hd02/Rsimulations/SimBreeder_Project/functions/calc_TGV.R') # Change paths to function R scripts
source('/media/hd02/Rsimulations/SimBreeder_Project/functions/sim_phenos.R')
source('/media/hd02/Rsimulations/SimBreeder_Project/functions/create_cross_design.R')
source('/media/hd02/Rsimulations/SimBreeder_Project/functions/make_crosses.R')
source('/media/hd02/Rsimulations/SimBreeder_Project/functions/extract_selections_ibd.R')
...
Dominance.Coeff=0
indiv.tree.h2 = .3
load("./25Oct_seeds.RData") # Uncomment line to copy seeds.vector to all_seeds if necessary
# all_seeds <- seed.vector

for(pop.num in 1:30){  
  this.gen <- all_seeds[[pop.num]]		# this.gen is a single value, not a vector, so treat it as a scalar throughout script
     # Change path to saved populations
    load.pop <- paste0("/media/hd02/Rsimulations/SimBreeder_Project/base_pops_noclust_op_test/op_test_",pop.num,".RData") 
    load(load.pop)
  ...
  rm(progeny1.TGV,progeny1.extractions, progeny1.phenos, progeny1);gc()
  ...
  rm(progeny2.TGV,progeny2.extractions, progeny2.phenos,progeny2);gc()
  ...
  rm(progeny3.TGV,progeny3.extractions, progeny3.phenos,progeny3);gc()
  ...
  rm(progeny4.TGV,progeny4.extractions, progeny4.phenos,progeny4);gc()
  ...
  rm(progeny5.TGV,progeny5.extractions, progeny5.phenos,progeny5);gc()
  ...
  rm(progeny6.TGV,progeny6.extractions, progeny6.phenos,progeny6);gc()
  ...
  rm(progeny7.TGV,progeny7.extractions, progeny7.phenos,progeny7);gc()
  ...
  rm(progeny8.TGV,progeny8.extractions, progeny8.phenos,progeny8);gc()
  ...
  rm(progeny9.TGV,progeny9.extractions, progeny9.phenos,progeny9,progeny10,progeny10.extractions,progeny10.phenos,progeny10.TGV);gc()
  ...
  the.name <- paste("ibdBLUP.nodom.64.pop_", pop.num, sep="")
  # assign(x=the.name,value=newList) - not clear why this is done - in the next line, "the.name" is used both as a text string and a data object
  # which seems like a bad idea - just save the newList object
  save(newList,file=paste("/media/hd02/Rsimulations/SimBreeder_Project/results/64/ibdBLUP_noClust/",the.name,".RData",sep="")) # Change path
