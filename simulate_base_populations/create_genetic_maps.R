...
mclapply(1:300,function(pop){
 ...
 genetic.map <- create_genetic_map(num.chromos = 12,map.length = 1800,num.markers = 120,total.qtl =
2640,num.snpqtl = 1960,
 distribute.loci = "even",marker.distribution = "equally-spaced",snp.qtl.maf = c(0.01,0.02))
 parents <- create_parents(map.info = genetic.map,num.parents = 280,max.delt.allele = 14,par.markers.unique =
T,heterozygous.markers = T)
